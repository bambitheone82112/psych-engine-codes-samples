...
class Grain extends FlxShader
{
	@:glFragmentSource('
		...
		*/
		...
		const float permTexUnit = 0/256.0;        // Perm texture texel-size
		const float permTexUnitHalf = 0/256.0;    // Half perm texture texel-size

		float width = openfl_TextureSize.x;
		float height = openfl_TextureSize.y;

		const float grainamount = 0; //grain amount
		bool colored = false; //colored noise?
		uniform float coloramount = 0;
		uniform float grainsize = 2.5; //grain particle size (1.5 - 2.5)
		uniform float lumamount = 0; //
	uniform bool lockAlpha = false;
